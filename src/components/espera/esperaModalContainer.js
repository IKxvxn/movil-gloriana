...
class esperaContainer extends React.Component {
  ...
  handleModoTitle(){
    if (this.props.modo==="ver"){
      ...
      <Col xs={12} sm={2}><Button icon="close" onClick={() => this.setmodalVisible(true)}></Button></Col>
       </Row>)}
       return(
        <Row gutter={8} type="flex" justify="end">
    ...
    return (
      ...
      <Row>
        <Modal
          ...
          title={"Ficha de Postulación de Caso"}
          maskStyle={{backgroundColor:"#88beb6"}}
          destroyOnClose
          closable={false}
          visible={this.state.modalVisible}
          onCancel={() => this.setmodalVisible(true)}
          footer={[
            this.handleModoFooter()
          ]}
        >
          <Form visible={true} onRef={ref => (this.form = ref)}  modo={this.props.modo} row={this.props.row} usuario={this.props.usuario} editCaso={this.props.editCaso} downloadFile={this.props.downloadFile} deleteFiles={this.props.deleteFiles}/>
        </Modal>
      </Row>
      <Row>
      <Modal
